...
- [ ] CI: fix the v0.1.0 run — it failed in `beforeBuildCommand` with `Error: ghcup not found`. `haskell-actions/setup@v2` provides ghc/cabal on PATH but does NOT leave a `ghcup` binary where `bundle/fetch-ghc.sh` looks (`command -v ghcup` / `~/.ghcup/bin/ghcup`). Fix options: (a) add a workflow step to install ghcup non-interactively (`BOOTSTRAP_HASKELL_NONINTERACTIVE=1`) and push `~/.ghcup/bin` onto `$GITHUB_PATH`; or (b) set `GHCUP_CMD`/`CABAL_CMD` envs the script honors; or (c) make fetch-ghc.sh fall back to a PATH `ghc`/`cabal` when ghcup is absent. Then re-tag or re-dispatch. After green: confirm a `.dmg` lands on the draft Release + the `vendor` cache behaves on re-run.
